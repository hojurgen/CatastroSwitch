---
name: Resume Phase
description: "Use when: sync the runtime fork to the locked CatastroSwitch phase lane, resume an active phase from the current branch or phase-state artifact, identify the next ready step, and continue the workflow loop."
...
Before the resume summary, run `powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\scripts\sync-phase-workflow-lane.ps1 -Apply` from the control repo. If the user named a phase explicitly, pass `-Phase <phase-id>`.
...
- Use the sync helper output as the live runtime branch and worktree context before you infer the next handoff.
