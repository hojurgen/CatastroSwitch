...
7. Ask the Planner to reconcile current implementation versus the selected phase.
8. Use the Planner output to identify:
   - sequential tasks
   - parallel task groups
   - docs and validation required for each task
9. For each ready task:
...
10. Update the phase state artifact in the runtime fork checkout after planning, after each review, and after the Gatekeeper result.
11. Do not run the Gatekeeper until every task in the phase has a Reviewer `Pass`.
12. Run the Gatekeeper on the whole phase branch.
13. If the Gatekeeper returns `Error`, send the result back to the Planner and continue the loop.
14. If the Gatekeeper returns `Pass`, prepare the phase handoff and next-phase recommendation, and clear the active lock.
