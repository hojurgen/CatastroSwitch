...
6. Keep the phase-state `executionLock` current so the active agent, task, branch, worktree, and next handoff are explicit.
7. Confirm the selected phase branch has been replayed onto the runtime fork clean sync branch after any material upstream movement.
8. Ask the Planner to reconcile current implementation versus the selected phase.
9. Use the Planner output to identify:
   - sequential tasks
   - parallel task groups
   - upstream rebase hotspots
   - docs and validation required for each task
10. For each ready task:
...
11. Update the phase state artifact in the runtime fork checkout after planning, after each review, and after the Gatekeeper result.
12. Do not run the Gatekeeper until every task in the phase has a Reviewer `Pass`.
13. Run the Gatekeeper on the whole phase branch.
14. If the Gatekeeper returns `Error`, send the result back to the Planner and continue the loop.
15. If the Gatekeeper returns `Pass`, prepare the phase handoff and next-phase recommendation, and clear the active lock.
